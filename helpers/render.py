...
(helpers/repos.json) plus the exclusion sets — helpers/out_of_scope.json (auto:
not AI/Claude) and helpers/filtered.json (manual: in-scope but redundant) — then:
  1. drops repos whose GitHub description is primarily non-Latin, recording them in
     helpers/non_english.json (auto: the published list is English-language);
  2. selects the top `render.render_count` repos by stars that are neither excluded
     nor archived, and writes that set to helpers/repos_to_render.json;
  3. categorizes, builds the short briefs (from description / topics), and lays out
...
DEFAULT_NON_ENGLISH = HERE / "non_english.json"
...
def non_latin_share(text: str) -> float:
    """Share of a string's *letters* that are written in a non-Latin script.
    Digits, punctuation and emoji are not letters, so they neither pad the
    denominator nor dilute a fully non-English description."""
...
def select_non_english(universe: list[dict], excluded: set[str],
                       threshold: float) -> list[dict]:
    """Otherwise-publishable repos whose GitHub description is primarily non-Latin.

    The published list is English-language, and nothing upstream enforces that:
    scope_filter asks only whether a repo is AI/Claude-related, so a CJK-described
    repo sails into the top N on stars alone. This is that rule's gate, sited where
    the repo actually becomes visible.

    Ceiling: description only. The rule also covers a primarily non-English README,
    but reading one needs the network and render never networks — that half stays
    with the manual CLASSIFICATION AUDIT (audit_language.py). Every case observed so
    far was a description hit; a repo with an English description and a non-English
    README is what would force the upgrade.
    """
    hits = []
    for r in universe:
        if r.get("archived") or r.get("full_name", "").lower() in excluded:
            continue
        share = non_latin_share(r.get("description") or "")
        if share >= threshold:
            hits.append({"repo_id": r["full_name"], "share": round(share, 3)})
    return hits


def build_non_english_payload(hits: list[dict], threshold: float,
                              generated_at: str) -> dict:
    return {
        "description": (
            "Auto-generated by helpers/render.py: otherwise-publishable repos whose "
            "GitHub description is >= threshold non-Latin letters — the published "
            "list is English-language. Regenerated every run; DO NOT hand-edit "
            "(tune render.non_english_threshold in config.json). Repos excluded as "
            "off-topic live in helpers/out_of_scope.json; editorial exclusions in "
            "helpers/filtered.json."),
...
def main() -> int:
    ...
    ap.add_argument("--non-english", type=Path, default=DEFAULT_NON_ENGLISH)
    ...
    # universe - out_of_scope (auto) - filtered (manual) - non-English (auto)
    # - archived, top N by stars
    ...
    threshold = cfg["render"]["non_english_threshold"]
    non_english = select_non_english(universe, excluded, threshold)
    excluded |= {e["repo_id"].lower() for e in non_english}
    ...
    write_atomic(build_non_english_payload(non_english, threshold, generated_at),
                 args.non_english)
    ...
    log(f"wrote {args.out} + {args.render_out} ({len(render_set)} repos) "
        f"+ {args.non_english} ({len(non_english)} non-English)")
    return 0
